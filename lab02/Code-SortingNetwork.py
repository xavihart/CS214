...
parser.add_argument("-n", "--n", type=int, help="input n")
...
if __name__ == '__main__':
    ...
    k = args.n
    n_ = 2 * k
    sortingNetwork = Sorter_new(n_)
 

    winW, winH = 2400 * 0.3, 1500 * 0.3
    hMargin, vMargin = winW // 20, winH // 20
    hScale, vScale = (winW - 2 * hMargin) // sortingNetwork.hNum(), (winH - 2 * vMargin) // (n_ - 1)
    print("sortingNetwork number:", sortingNetwork.hNum())
    root = Tk()
    root.title('Sorting Network with 2n=%d (Drawn by Python Tkinter)' % n_)
