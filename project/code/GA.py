...
from gaTools import *
from tools.datatransfer import *
...
### split if
demand= [0] + list(dataMat[100])
locations = [(0, 0)] + [(lonArray[i], latArray[i]) for i in range(num_des)]
print(locations)
print(demand)
loc_spl = location_split(locations, demand)

split_Num = 4

split_ten = split_rand_gen(3)
locations,  demand = loc_spl.split(split_ten)

lonArray = [locations[i][0] for i in range(1,len(locations))]
latArray = [locations[i][1] for i in range(1,len(locations))]
demand = demand[1:]
...
def CrossOver(Si_, Sj):
    ...
    assert(cal_des_num(Si) == len(lonArray))
    return Si
...
def Muten(S):
    ...
    assert(cal_des_num(S) == len(lonArray))
    return S
...
def repairing(routes):
    ...
    if route_dmd(routes[r_max_i]) > cap:
        ...
        assert (cal_des_num(routes) == len(lonArray))
        return True
    return False
...
def main():
        ...
        for i in range(0, len(population) // 8):
            ...
            if r < cross_prop:
                ...
                if rmuta < muta_prop:
                   c = Muten(c)
                   #print("mt")
                assert (cal_des_num(c) == len(lonArray))
                SIA(c)
                f = repairing(c)
                population[-k] = c
                #print(cost(c))
    ...
    plt.plot(min_list)
    plt.savefig("./results/GA/{}/rand/BIG2costCurve_cap{}[alpha{}population{}]split{}.jpg".format(city, cap, alpha, population_number, split_Num))
    f=open("./results/GA/{}/rand/BIG2min_list_cap{}[alpha{}population{}]split{}.txt".format(city, cap, alpha, population_number, split_Num), "w")
    for i in min_list:
        f.write(str(i)+" ")
    f.write("\nroute:\n")
    for i in min_indiv:
...
if __name__ == '__main__':
    print("cap", cap)
    print(len(lonArray), lonArray)
    print(len(latArray), latArray)
    print(len(demand), demand)
    main()
