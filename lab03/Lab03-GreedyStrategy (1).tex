...
\begin{enumerate}
    ...
    \begin{proof}
       ...
       If the optimal answer doesn't satisfy the increasing order of $a_i \times b_i$, we think about the 
       adjacent pair, normally $T_i, T_{i+1}$ for $i \in \{0, 1, ..., n-1\}$, where $T_i = (a_i, b_i)$ and $T_{i+1} = (a_{i+1}, b_{i+1})$.
